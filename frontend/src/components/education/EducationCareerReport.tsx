...
function ClusterBanner({ chartType }: { chartType: ChartType }) {
  ...
  return (
    <div className="flex flex-wrap gap-4 items-start bg-linear-to-br from-sky-50 to-sky-100 border-[1.5px] border-sky-400 rounded-2xl p-5 mb-5">
...
function SnapshotCard({
  ...
  const cells: Array<[string, string | undefined]> = [
    ["Best UG Route", snapshot.best_ug_route],
    ["Strong Backup", snapshot.strong_backup_route],
    ["Best PG Route", snapshot.best_pg_route],
    ["Career Cluster", snapshot.best_career_cluster],
  ];
  const shown = cells.filter(([, v]) => v);
  if (!identity.one_line_summary && !shown.length) return null;

  return (
    <section className="rounded-2xl border border-slate-200 bg-white shadow-sm p-6 mb-6">
      {identity.macro_identity ? (
        <div className="text-[11px] font-bold uppercase tracking-[0.12em] text-teal-700 mb-2">
          {identity.macro_identity}
          {identity.confidence ? (
      ...
      {identity.one_line_summary ? (
        <p className="text-[1.05rem] leading-relaxed text-slate-800 mb-4">
          {identity.one_line_summary}
        </p>
      ) : null}
      {shown.length ? (
        <div className="grid grid-cols-2 md:grid-cols-4 gap-3">
          {shown.map(([label, value]) => (
            <div key={label} className="rounded-xl border border-slate-200 bg-slate-50 px-3 py-2.5">
              <div className="text-[0.7rem] font-bold uppercase tracking-wider text-slate-400 mb-1">
                {label}
              </div>
              <div className="text-[0.95rem] font-bold text-slate-900 leading-snug">{value}</div>
            </div>
          ))}
        </div>
      ) : null}
...
function MacroClusters({ clusters }: { clusters: MacroCluster[] }) {
  if (!clusters.length) return null;
  return (
    <section className="mb-6">
      <div className="text-[0.85rem] font-bold tracking-widest uppercase text-slate-500 mb-3">
        Macro Career Clusters
      </div>
      <div className="grid grid-cols-1 md:grid-cols-2 gap-3">
        {clusters.slice(0, 6).map((c, i) => {
          const pct = Math.max(0, Math.min(100, Math.round(c.strength_pct ?? 0)));
          return (
            <div
              key={c.cluster ?? i}
            ...
            >
              <div className="flex items-baseline justify-between gap-2 mb-2">
                <span className="font-bold text-slate-900 text-[0.98rem] leading-snug">
                  {c.rank ? `${c.rank}. ` : ""}
                  {c.cluster}
                </span>
                <span className="text-teal-700 font-extrabold text-sm shrink-0">{pct}%</span>
              </div>
              ...
              {c.member_fields?.length ? (
                <p className="text-[0.8rem] text-slate-500 leading-snug">
                  {c.member_fields.slice(0, 5).join(" · ")}
                </p>
              ) : null}
            </div>
          );
...
export function EducationCareerReport({ data }: Props) {
  ...
  const report = (data.report ?? {}) as { final_identity?: FinalIdentity; snapshot?: Snapshot };
  ...
  return (
    <div className="max-w-[1280px] mx-auto text-slate-800">
      <header className="text-center mb-8 pb-5 border-b-2 border-slate-200">
      ...
      <SnapshotCard identity={identity} snapshot={snapshot} />
      ...
      <MacroClusters clusters={macroClusters} />
      ...
      <div className="text-center text-[0.85rem] font-bold tracking-widest uppercase text-slate-500 mb-6">
        Top {Math.min(TOP_N, topFields.length)} Fields
      </div>
