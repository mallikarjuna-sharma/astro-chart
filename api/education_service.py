...
def _ensure_ai(hit: dict[str, Any]) -> dict[str, Any]:
    """Legacy rows were saved before AI diagnostics existed.

    Missing ``AI`` must not trigger a full engine recompute.
    """
    if hit.get("AI") is not None:
        return hit
    out = dict(hit)
    out["AI"] = ai_status(error="AI diagnostics were not stored for this analysis.")
    return out
